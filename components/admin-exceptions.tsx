...
export function AdminExceptions({ initial, barbers, onChanged }: { initial: Exception[]; barbers: Barber[]; onChanged: () => Promise<void> }) {
  ...
  return (
    <section className="mt-8 overflow-hidden rounded-sm border border-border bg-card p-5">
      ...
      <form onSubmit={submit} className="mt-5 flex flex-col gap-4">
        <div className="grid min-w-0 gap-4 md:grid-cols-2 lg:grid-cols-4">
          <label className="flex min-w-0 flex-col gap-1 text-xs text-muted-foreground">
            Barbeiro
            <select required value={form.barberId} onChange={(e) => setForm({ ...form, barberId: e.target.value })} className="min-h-11 w-full min-w-0 rounded-sm border border-border bg-background px-3 text-base text-foreground">
          ...
          <label className="flex min-w-0 flex-col gap-1 text-xs text-muted-foreground">
            Data inicial
            <input required type="date" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} className="min-h-11 w-full min-w-0 rounded-sm border border-border bg-background px-3 text-base text-foreground" />
          </label>
          <label className="flex min-w-0 flex-col gap-1 text-xs text-muted-foreground">
            Data final
            <input type="date" min={form.startDate} value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} className="min-h-11 w-full min-w-0 rounded-sm border border-border bg-background px-3 text-base text-foreground" />
          </label>
          <label className="flex min-w-0 flex-col gap-1 text-xs text-muted-foreground">
            Descrição
            <input required placeholder="Ex.: Férias" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="min-h-11 w-full min-w-0 rounded-sm border border-border bg-background px-3 text-base text-foreground" />
          </label>
        </div>
        <div className="flex flex-col gap-3 rounded-sm border border-border/70 bg-background/50 p-3 sm:flex-row sm:flex-wrap sm:items-center">
          ...
          {!form.closed && <div className="grid min-w-0 flex-1 grid-cols-2 gap-3 sm:max-w-sm"><label className="flex min-w-0 flex-col gap-1 text-xs text-muted-foreground">Abre<input required type="time" value={form.startTime} onChange={(e) => setForm({ ...form, startTime: e.target.value })} className="min-h-11 w-full min-w-0 rounded-sm border border-border bg-background px-3 text-base text-foreground" /></label><label className="flex min-w-0 flex-col gap-1 text-xs text-muted-foreground">Fecha<input required type="time" value={form.endTime} onChange={(e) => setForm({ ...form, endTime: e.target.value })} className="min-h-11 w-full min-w-0 rounded-sm border border-border bg-background px-3 text-base text-foreground" /></label></div>}
