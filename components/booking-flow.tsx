...
export function BookingFlow({ barbers, services }: Props) {
  ...
  useEffect(() => {
    const element = bookingRootRef.current
    if (!element) return
    const observer = new IntersectionObserver(([entry]) => setIsBookingVisible(entry.isIntersecting), { threshold: 0.05 })
  ...
  return <div ref={bookingRootRef} className="mx-auto max-w-3xl">
